---
title: 'Audition Information for Bells of the Rockies'
featured_image: '/images/bg-handbells.jpg'
header_section_class: 'tc-l pv2 ph3 ph4-ns fw6'
---

{{< callout full-ringers >}}
We have enough ringers at this time. Next auditions will be held in May 2027.
{{< /callout >}}

{{< center >}}
## Join Our Bell Substitute List
{{< end-block >}}

We are always looking for new ringers to join our substitute roster. Being a sub is a great way to meet the group and try your hand at ringer with the ensemble, but without the weekly commitment. For more information, email bellsoftherockies1996@gmail.com.

***

{{<columns>}}
Bells of the Rockies is an auditioned community ensemble. If you would like information about future auditions for our group, please submit the audition request form. We will contact you the next time we announce audition information.

{{< center >}}
### What to Know About Future Auditions
{{< end-block >}}

Basic member requirements:

1. Be able to sight-read level 3 music (we will play more difficult music, but it is expected members can sight read at level 3 in a single rehearsal).
2. Be willing to commit for an entire season (August - May, with concerts in December and April/May) 
3. Be on time and attend all rehearsals. We rehearse on Tuesday evenings in Loveland, Colorado. (four absences per year allowed when a substitute is secured).

Future auditions may include the following components:

1. Present prepared exercises 
2. Sight read one selection 
3. On-site: complete two written exercises 
4. Interview with Bells of the Rockies Board director and up to two other adjudicators
{{<column>}}



# Bells of the Rockies Audition Sign ups

{{< form-custom action="#" >}}
   {{< field name="name" label="Your Name" required=true >}}
   {{< field name="email" type="email" label="Email Address" required=true hint="email@example.com" >}}
   {{< field name="phone" type="tel" label="Phone" required=true hint="000-0000" >}}
{{< /form-custom >}}



{{<end-block 2 true>}}
